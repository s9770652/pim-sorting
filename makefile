...
DPU_FLAGS := ${COMMON_FLAGS} -O2 -DNR_TASKLETS=${NR_TASKLETS} -DBLOCK_SIZE=${BLOCK_SIZE} -D${TYPE} -DSEQREAD_CACHE_SIZE=${SEQREAD_CACHE_SIZE} -DPERF=${PERF} -DCHECK_SANITY=${CHECK_SANITY} -DSTACK_SIZE_DEFAULT=512
